...
export async function backupCards(directoryPath: string): Promise<void> {
  const filePath = path.join(path.resolve(directoryPath), `/guru-cards-backup_${new Date().toJSON()}.json`)
...
export function handleCardsFilter(
  ...
  if (Object.keys(parsed).includes('boards') && !Object.keys(parsed).includes('collection')) {
    console.log('Please add "collection:example" to filter when filtering for boards')
    process.exit()
  }
  ...
  if (options?.raw) {
    const rawCardsFiltered = cards.map((card) => cardsRaw.find((cardRaw) => cardRaw.id === card.id)) as any
