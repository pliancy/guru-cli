...
export default async (cli: any): Promise<void> => {
    cli.command(
        ...
        .action(async (find: string, replace: string, filter: string, options: any) => {
            ...
            if (!options.confirm) {
                ...
                for (const card of matchedCards) {
                    ...
                    for (const match of matches) {
                        ...
                        console.log(
                            `${beforeText}${chalk.bold.redBright(matchText)}${chalk.bold.green(
                                replace,
                            )}${afterText}`,
                        )
                    }
            ...
            console.log(
                `Successfully replaced "${chalk.bold.redBright(find)}" with "${chalk.bold.green(
                    replace,
                )}" across ${matchedCards.length} cards.`,
            )
        })
}
