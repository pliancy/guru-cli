...
export default async (cli: any): Promise<void> => {
  cli
    ...
    .option('-C, --confirm', 'confirm making the changes (required to push modifications to guru)')
    .option('-w, --preview-width <size>', 'characters of text around the match when in preview mode (no --confirm)')
    .action(async (find: string, replace: string, filter: string, options: any) => {
      ...
      const filteredCards = handleCardsFilter(filter, cardsRaw, options, { raw: true }) as GuruCardRaw[]
      let reFlags = 'g'
      if (options.ignoreCase) reFlags += 'i'
      const matchedCards = filteredCards.filter((card) => new RegExp(find, reFlags).test(card.content))
      ...
      if (!options.confirm) {
        ...
        for (const card of matchedCards) {
          ...
          const matches = [...card.content.matchAll(new RegExp(find, reFlags))]
          if (!matches.length) throw new Error(`Unknown regex matching error occurred on card ${card.preferredPhrase}`)
          for (const match of matches) {
            const matchText = match[0] as string
            const matchIndex = match.index as number
            const SIZE_AROUND = options.previewWidth ? Number(options.previewWidth) : 40
            ...
            const afterText = card.content.substring(
              ...
              matchIndex + matchText.length + SIZE_AROUND >= card.content.length - 1
                ? card.content.length - 1
                : matchIndex + matchText.length + SIZE_AROUND,
            )
            console.log(`${beforeText}${chalk.bold.redBright(matchText)}${chalk.bold.green(replace)}${afterText}`)
          }
          console.log('')
        }
        console.log(
      ...
      if (!options.force) {
        ...
        const confirm = await cliux.confirm(
          `This will replace all instances matching "${chalk.bold.redBright(find)}" with ${chalk.bold.green(
            replace,
          )} in ${matchedCards.length} cards. Continue? (Y/n)`,
        )
        if (!confirm) return
      }

      for (const card of matchedCards) {
        card.content = card.content.replace(new RegExp(find, reFlags), replace)
        await guru.updateCardRaw(card)
      }
      console.log(
        `Successfully replaced "${chalk.bold.redBright(find)}" with "${chalk.bold.green(replace)}" across ${
          matchedCards.length
        } cards.`,
      )
    })
