...
export default async (cli: any): Promise<void> => {
  cli
    .command('verify-by-title <title>', 'Verifies cards by card title.')
    ...
    .action(async (title: string, options?: any) => {
      ...
      let filteredCards = handleCardsFilter(`title:"^${title}$"`, cardsRaw, '-i') as GuruCard[]

      if (options.collection) {
        filteredCards = filteredCards.filter((card) => card.collection === options.collection)
      }
      ...
      filteredCards.map((card) =>
        console.log(`${chalk.bold.blueBright(card.collection)} ${chalk.bold.greenBright(card.title)} (${card.link})`),
      )

      const confirm = await cliux.confirm('Do you wish to verify the identified card(s)? (Y/n)')
      if (!confirm) return
