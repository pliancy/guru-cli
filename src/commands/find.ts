...
export default async (cli: any): Promise<void> => {
  cli
    .command('find <filter>', 'find cards based on search filter (full ECMAScript compliant regex supported)')
    .example(
...
`,
    ...
    .action(async (filter: string, options: any) => {
      ...
      cliux.action.stop(`✅  ${filteredCards.length} cards matching your filter\n---------------------------`)
      filteredCards.map((card) => console.log(`${chalk.bold.blueBright(card.title)} (${card.link})`))
    })
}
