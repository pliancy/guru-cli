...
export default async (cli: any): Promise<void> => {
  cli
    .command('verify-expired [filter]', 'Verifies all expired cards with optional filtering (see README)')
      ...
      const progress = new Progress.Bar(
        {
          format: `Verifying Expired Cards ${chalk.hex('#7F4BAE')(
            '{bar}',
          )} {percentage}% | Current Card: {value}/{total} | Name: ${chalk.bold.hex('#D7E150')('{currentCard}')}`,
        },
